...
def train_and_evaluate_multiple_runs():
    ...
    for seed in seeds:
        ...
        training_params = {
            ...
            'patience': 10,
            'project': 'runs/detect',
            'name': f'yolov_maritime_seed_{seed}',
            'exist_ok': True,
            'amp': True,
            'workers': 0,
            # 'pin_memory': False,
        ...
        run_result = {
            ...
            'precision': float(metrics.box.p.mean()) if hasattr(metrics.box, 'p') and metrics.box.p is not None else 0.0,
            'recall': float(metrics.box.r.mean()) if hasattr(metrics.box, 'r') and metrics.box.r is not None else 0.0,
